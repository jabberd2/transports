...
sub sendSmsViaOrange
{
    ...
    if ($res->content =~ /<span class=\"label\">z do..adowa..:<\/span>\s*<span class=\"value\">([0-9]+)<\/span>/) { $sms_zostalo = $sms_zostalo + eval ($1); }
    ...
    if ($cnt =~ /<span class=\"label\">z do..adowa..:<\/span>\s*<span class=\"value\">([0-9]+)<\/span>/) { $sms_zostalo2 = $sms_zostalo2 + eval ($1); }
